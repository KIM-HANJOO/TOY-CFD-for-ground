...
condnums = startnums + ones(length_duct_now, 1);
x = zeros(2, 1);
for i = 1 : length_duct_now
    x(1, 1) = 18;
    x(2, 1) = condnums(i, 1);
...
%%%%
load('duct_all.mat')
duct_all(N_weather * (num_duct_now - 1) + 1 : N_weather * (num_duct_now - 1) + N_weather, 1 : 3) = weather(:, 1 : 3);
duct_all(N_weather * (num_duct_now - 1) + 1 : N_weather * (num_duct_now - 1) + N_weather, 4 : 3 + N_node_all) = T_all(:, 4 : end);
