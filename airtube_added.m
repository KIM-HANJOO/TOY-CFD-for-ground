...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% airtube 추가하기, info_g 덮어쓰기
[airtube_numbers, info_g] = airtubes(X, info_g);
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% inlet과 outlet 위에도 서페이스 추가하기 (자동추가)
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% type 5인 애들은 따로 만들어주기

for i = 1 : max(size(airtube_numbers(:, 1)))
    Z = airtube_numbers;
    x(1, 1) = Z(i, 1);
    z = Z(i, 1); % 지금 선택한 노드 번호
    
    if i == 1 %% inlet (인접 노드 한개)
        x1 = Z(i + 1, 1); %% 인접한 airtube 골라주기
        
        for j = 1 : 6 %% airtube 말고 나머지랑 cond 추가
        x(2, 1) = info_g(z, j + 4);
        if info_g(z, j + 4) ~= x1
            Se_g = S_cond;
            for g=1:2; h=1:2;
        ...
        elseif info_g(z, j + 4) == x1
            Se_g = S_conv_airtubes;
            Se_v = S_infiltration;
            for g=1:2; h=1:2;
                S_g(x(g,1),x(h,1)) = S_g(x(g,1),x(h,1)) + Se_g(g,h);
                S_g(x(g,1),x(h,1)) = S_g(x(g,1),x(h,1)) + Se_v(g,h);
            end
        end
        end
    ...
    elseif i == max(size(Z(:, 1))) %% outlet (인접 노드 한개)
        x1 = Z(i - 1, 1);
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% inlet 과 16번(T_out) 사이 열교환 추가
N_adj_now = zeros(2, 1);
x = N_adj_now;

x(1, 1) = airtube_numbers(1, 1);
x(2, 1) = N_node_g2 - 1;
...
for g=1:2; h=1:2;
    S_ult(x(g,1),x(h,1)) = S_ult(x(g,1),x(h,1)) + Se_g(g,h);
    S_ult(x(g,1),x(h,1)) = S_ult(x(g,1),x(h,1)) + Se_v(g,h);
end

% outlet 과 17번(T_indoor) 사이 열교환 추가
N_adj_now = zeros(2, 1);
x = N_adj_now;

x(1, 1) = airtube_numbers(end, 1);
x(2, 1) = N_node_g2;
